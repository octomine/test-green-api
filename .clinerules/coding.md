...
- **Всегда** `cn(...)`, включая случаи с одним условием.
- **Никогда** не использовать шаблонные строки (`` `${...}` ``) для классов — ни однострочные, ни многострочные.
- **Никогда** не использовать многострочные шаблонные строки с отступами — они оставляют пробелы и переводы строк в атрибуте `class`.
...
- **`i18next` + `react-i18next`** — единственное решение для текстов.
- Никаких самодельных словарей, `react-intl`, `@lingui/*` и т.п.
- Не устанавливать `i18next-http-backend`, `i18next-browser-languagedetector` — переводы грузятся через `import.meta.glob`.

### Где живёт

- Модуль: `src/shared/i18n/`.
- Тексты: `src/shared/i18n/locales/<lang>.json`.
- Язык сейчас один — `ru`. Второй не добавлять без явной просьбы.
...
- **Никогда** не хардкодить пользовательские строки в JSX, атрибутах, `title`, `placeholder`, `aria-label`.
  - ✅ `<Button>{t('auth.submit')}</Button>`
  - ❌ `<Button>Войти</Button>`
- **Всегда** использовать ключи из `ru.json`.
  - ✅ `t('auth.idInstanceRequired')`
  - ❌ `t('ID Instance обязателен')` (нельзя передавать текст как ключ)
- **Интерполяция** — через `{{var}}` в JSON и объект в `t()`:
  - В JSON: `"instanceLabel": "Инстанс: {{id}}"`
  - В коде: `t('chat.instanceLabel', { id: credentials.idInstance })`
  - **Не** использовать стрелочные функции или конкатенацию строк.
- **Плюрализация** — через суффиксы `_one` / `_few` / `_many` при необходимости (пока не используется).

### Структура `ru.json`

- **Верхний уровень** — секции по фичам/страницам: `auth`, `chat`, `createChat`, `messages`, и т.д.
- **Вложенность** внутри секции — плоская, максимум 2 уровня.
- **Ключи** — camelCase, описательные:
  - ✅ `idInstanceRequired`, `stubTitle`, `logout`
  - ❌ `err1`, `btn`, `msg`
- **Секция создаётся вместе с фичей/страницей**, а не заранее.
- Порядок ключей — по смыслу использования в UI (сверху вниз).

### Добавление нового текста

1. Открыть `src/shared/i18n/locales/ru.json`.
2. Добавить ключ в подходящую секцию (создать секцию, если фича новая).
3. Использовать в компоненте через `t('section.key')`.
4. **Не** создавать параллельные файлы локализации внутри фич (`features/auth/locales/ru.json` — запрещено). Все тексты в одном месте.

### Типизация

- `src/shared/i18n/i18next.d.ts` расширяет `CustomTypeOptions` через `typeof ru`.
- После добавления нового ключа TypeScript сам подхватит его — никаких ручных деклараций не нужно.
- Опечатки в ключах (`t('auth.titl')`) ловятся на этапе компиляции.

### Чего НЕ делать

- Не устанавливать i18n-библиотеки без явной просьбы.
- Не добавлять провайдеры, контексты, обёртки над `useTranslation`.
- Не добавлять переключатель языка — язык фиксирован (`ru`).
- Не использовать `i18n.t()` напрямую (только через `useTranslation` в компонентах).
- Не выносить тексты в отдельные константы (`const TITLE = 'Вход'`) — только в JSON.
